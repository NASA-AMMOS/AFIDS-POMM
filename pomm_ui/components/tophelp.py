...
def TopHelp(self, name):
    headH = 30
    self.helpIcon = PhotoImage(data=importlib.resources.open_binary(
        "pomm_ui.assets", "help.png").read())
    buttonHelp = Button(
        self,
        image=self.helpIcon,
    ...
    if name == 'pomm':
        ...
        self.homeIcon = PhotoImage(data=importlib.resources.open_binary(
            "pomm_ui.assets", "home.png").read())
        buttonHome = Button(
            self,
            image=self.homeIcon,
