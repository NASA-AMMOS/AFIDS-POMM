...
class Command(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        for index, (text, val, page, desc, req) in enumerate(COMMANDS):
            ...
            img = PhotoImage(data=importlib.resources.open_binary("pomm_ui.assets", f"{val}.png").read())
            button = Button(
                card,
                image=img,
                command=select(page)
            ...
            self.helpIcons[index] = PhotoImage(data=importlib.resources.open_binary("pomm_ui.assets", "help.png").read())
